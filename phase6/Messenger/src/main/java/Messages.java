...
public class Messages {
    ...
    public static void sendMessage(Session session, int destination, String userName, String filePath, String message) throws IOException {
        //condition
...
//            System.out.println("U ARE NOT IN THIS CHAT");
        ...
        messages.attachedFile = File.addFile(session, filePath);
    ...
    public static void editMessage(Session session, int messageID, String newText) {
        ...
        messages.setTextMessage(newText);
        Main.Update(session, messages);
    }
    ...
    public static Long getNumMessagesFromOneUser(Session session, String userName) {
    ...
    public static double getAvgNumberMessagesOfOneUser(Session session, String userName) {
        Query query = session.createQuery("select count(*) FROM Messages where Sender.UserName = :username", Messages.class);
        query.setParameter("username", userName);
    ...
    public static List<User> getViewersOfOneMessage(Session session, int messageID) {
        Messages message = session.get(Messages.class, messageID);
        Chat chat = message.getDestination();
        Query query = session.createQuery("select m.user from Member m where m.chat =: chat and m.lastSeenMessage >= :lastSeen", User.class);
        query.setParameter("chat", chat);
        query.setParameter("lastSeen", messageID);
